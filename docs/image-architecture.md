...
- NVIDIA split into explicit `open` and `legacy` streams, plus GNOME-only extras
...
- `recipes/layers/shared/nvidia-gnome.yml`: GNOME-only NVIDIA userspace extras and display-oriented kernel args
- `recipes/layers/alma9/core.yml` and `recipes/layers/alma10/core.yml`: distro-specific core deltas only
- `recipes/layers/alma9/gnome.yml` and `recipes/layers/alma10/gnome.yml`: distro-specific GNOME deltas on top of the shared `gnome-base.yml` layer

## Build Flow

The build workflow is ordered so GNOME images wait for the full core image job.

- core images are published first
- GNOME recipes use `ghcr.io/myos-dev/core-full-*` as their `base-image`
- GNOME rebuilds therefore track the published full core image line instead of reassembling the whole core stack inside the GNOME recipe
- NVIDIA GNOME images add `nvidia-gnome.yml` on top of the published NVIDIA core images for the matching stream
...
- `gnome-*` now reuses that same shared per-user OpenClaw runtime model instead of adding a separate session-bound helper through `/etc/skel`.
...
GNOME short-name mapping is now:

- `alma9` -> `gnome-alma9`
- `alma9-nvidia-open` -> `gnome-alma9-nvidia-open`
- `alma9-nvidia-legacy` -> `gnome-alma9-nvidia-legacy`
- `alma10` -> `gnome-alma10`
- `alma10-nvidia-open` -> `gnome-alma10-nvidia-open`
...
- `core-full-*` is the right default parent for GNOME images
- base NVIDIA images should stay leaner than GNOME NVIDIA images
...
- whether the published `latest` core tags are the right GNOME base tags for your PR workflow expectations
