...
PAYLOAD_REF="${CURRENT_INSTALLER_PAYLOAD_REF:-${MYOS_INSTALLER_PAYLOAD_REF:-ghcr.io/pelagians/alma10-server:latest}}"
INSTALLER_IMAGE="${CURRENT_INSTALLER_IMAGE:-${MYOS_INSTALLER_IMAGE:-localhost/current-alma10-server-installer:dev}}"
BUILDER_IMAGE="${BOOTC_IMAGE_BUILDER:-quay.io/centos-bootc/bootc-image-builder:latest}"
ROOTFS="${CURRENT_INSTALLER_ROOTFS:-${MYOS_INSTALLER_ROOTFS:-xfs}}"
OUTPUT_DIR="${CURRENT_INSTALLER_OUTPUT_DIR:-${MYOS_INSTALLER_OUTPUT_DIR:-$REPO_ROOT/output/installer/alma10-server}}"
CONFIG="${CURRENT_INSTALLER_CONFIG:-${MYOS_INSTALLER_CONFIG:-$REPO_ROOT/installer/alma10-server/iso.toml}}"
CONTAINERFILE="${CURRENT_INSTALLER_CONTAINERFILE:-${MYOS_INSTALLER_CONTAINERFILE:-$REPO_ROOT/installer/alma10-server/Containerfile}}"
PODMAN="${PODMAN:-podman}"
SUDO="${SUDO:-sudo}"
OUTPUT_OWNER="${CURRENT_INSTALLER_OUTPUT_OWNER:-${MYOS_INSTALLER_OUTPUT_OWNER:-$(id -u):$(id -g)}}"
